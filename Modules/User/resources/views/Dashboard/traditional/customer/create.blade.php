...
@section('content')
    <div class="user-create-container">
        ...
        <form class="form-container" id="userForm" method="POST"
            action="{{ LaravelLocalization::localizeURL(route('dashboard.customer.store')) }}">
            @csrf
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        ...
                        <input type="tel" class="form-input @error('phone') is-invalid @enderror" name="phone"
                            placeholder="+966501234567" value="{{ old('phone') }}">
                        @error('phone')
                            <div class="form-error">{{ $message }}</div>
                        @enderror
                ...
                <div class="form-grid">
                    <div class="form-group">
                        ...
                        <input type="password" class="form-input @error('password') is-invalid @enderror" name="password"
                            id="password" placeholder="{{ __('Strong password') }}" required>
                        @error('password')
                            <div class="form-error">{{ $message }}</div>
                        @enderror
                        <div class="password-strength" id="passwordStrength">
                            ...
                            <span id="strengthText">{{ __('Enter password') }}</span>
                        </div>
                    </div>
                    <div class="form-group">
                        ...
                        <input type="password" class="form-input @error('password_confirmation') is-invalid @enderror"
                            name="password_confirmation" placeholder="{{ __('Re-enter password') }}" required>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-select @error('role') is-invalid @enderror" name="role" required>
                            ...
                            <option value="customer" {{ old('role') == 'customer' ? 'selected' : '' }}>
                                {{ __('Customer') }}</option>
                            <option value="staff" {{ old('role') == 'staff' ? 'selected' : '' }}>{{ __('Staff') }}
                            </option>
                            <option value="owner" {{ old('role') == 'owner' ? 'selected' : '' }}>{{ __('Store Owner') }}
                            </option>
                        </select>
                        @error('role')
                            <div class="form-error">{{ $message }}</div>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-select @error('group_id') is-invalid @enderror" name="group_id">
                            ...
                            @foreach (\Modules\User\Models\Group::all() as $group)
                ...
                <div class="form-grid">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-input @error('address') is-invalid @enderror" name="address"
                            placeholder="{{ __('Full address') }}" value="{{ old('address') }}">
                        @error('address')
                            <div class="form-error">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-input @error('postal_code') is-invalid @enderror"
                            name="postal_code" placeholder="12345" value="{{ old('postal_code') }}">
                        @error('postal_code')
                            <div class="form-error">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="form-group">
                        ...
                        <select class="form-select @error('country') is-invalid @enderror" name="country">
                            ...
                            <option value="SA" {{ old('country', 'SA') == 'SA' ? 'selected' : '' }}>
                                {{ __('Saudi Arabia') }}</option>
                            <option value="AE" {{ old('country') == 'AE' ? 'selected' : '' }}>{{ __('UAE') }}
                            </option>
                            <option value="KW" {{ old('country') == 'KW' ? 'selected' : '' }}>{{ __('Kuwait') }}
                            </option>
                            <option value="QA" {{ old('country') == 'QA' ? 'selected' : '' }}>{{ __('Qatar') }}
                            </option>
                            <option value="BH" {{ old('country') == 'BH' ? 'selected' : '' }}>{{ __('Bahrain') }}
                            </option>
                            <option value="OM" {{ old('country') == 'OM' ? 'selected' : '' }}>{{ __('Oman') }}
                            </option>
                        </select>
                        @error('country')
                            <div class="form-error">{{ $message }}</div>
                ...
                <div class="form-grid">
                    <div class="form-group">
                        ...
                        <select class="form-select @error('language') is-invalid @enderror" name="language">
                            <option value="ar" {{ old('language', 'ar') == 'ar' ? 'selected' : '' }}>
                                {{ __('Arabic') }}</option>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-select @error('timezone') is-invalid @enderror" name="timezone">
                            <option value="Asia/Riyadh"
                                {{ old('timezone', 'Asia/Riyadh') == 'Asia/Riyadh' ? 'selected' : '' }}>
                                {{ __('Riyadh (GMT+3)') }}</option>
                            <option value="Asia/Dubai" {{ old('timezone') == 'Asia/Dubai' ? 'selected' : '' }}>
                                {{ __('Dubai (GMT+4)') }}</option>
                            <option value="Asia/Kuwait" {{ old('timezone') == 'Asia/Kuwait' ? 'selected' : '' }}>
            ...
            <div class="form-section">
                ...
                <div class="avatar-preview">
                    <div class="avatar-preview-img" id="avatarPreview">أ</div>
...
@push('scripts')
    <style>
        /* User Create Page Specific Styles */
    ...
    <script>
        // User create page specific JavaScript
        document.addEventListener('DOMContentLoaded', function() {
            ...
            form.addEventListener('submit', function(e) {
                ...
                if (!userData.name || !userData.email || !userData.password || !userData
                    ...
                    if (!userData.password_confirmation) missingFields.push('Password Confirmation');
                    if (!userData.role) missingFields.push('Role');
                    alert('{{ __('Please fill in all required fields') }}: ' + missingFields.join(', '));
                    return;
                }
                ...
                if (userData.password !== userData.password_confirmation) {
                    e.preventDefault();
                    alert('{{ __('Password and confirmation do not match') }}');
                    return;
                }
                ...
                submitBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> {{ __('Adding...') }}';
