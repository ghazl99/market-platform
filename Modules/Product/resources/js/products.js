...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Product actions - let view and edit links work naturally
    document.querySelectorAll('.action-btn.delete').forEach(btn => {
        btn.addEventListener('click', function(e) {
            // Only handle delete buttons, let view and edit work as normal links
            e.preventDefault();
            const productCard = this.closest('.product-card');
            const productName = productCard.querySelector('.product-name').textContent;

            console.log('Delete button clicked for:', productName);
            // Delete is handled by the main page script
        });
    });
