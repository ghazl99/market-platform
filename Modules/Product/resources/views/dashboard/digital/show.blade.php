        ...
        <div class="product-settings-section" id="settings-tab" style="display: none;">
            ...
            <form class="settings-form" method="POST" action="{{ route('dashboard.product.update', $product->id) }}">
                ...
                <input type="hidden" name="name"
                    value="{{ old('name', $product->getTranslation('name', app()->getLocale())) }}">
                <input type="hidden" name="description"
                    value="{{ old('description', $product->getTranslation('description', app()->getLocale())) }}">
                <input type="hidden" name="price" value="{{ old('price', $product->price) }}">
                <input type="hidden" name="category"
                    value="{{ old('category', $product->categories()->first()?->id) }}">
            ...
            <div class="attributes-section" style="margin-top: 2rem;">
                <div class="settings-header" style="margin-bottom: 1.5rem;">
                    <h2 class="settings-title">{{ __('Product Attributes') }}</h2>
                    <button type="button" class="settings-save-btn" id="add-attribute-btn" style="margin: 0;">
                ...
                <div class="attributes-list" id="attributes-list">
                    @forelse($product->attributes as $attribute)
                        @php
                            ...
                            $valueText = is_array($value) ? ($value[app()->getLocale()] ?? reset($value)) : ($attribute->pivot->value ?? '');
                        @endphp
                        <div class="attribute-item" data-attribute-id="{{ $attribute->id }}">
                            <div class="attribute-info">
                                <div class="attribute-name">
                                    <strong>{{ $attribute->getTranslation('name', app()->getLocale()) }}</strong>
                                    @if($attribute->unit)
                                        <span class="attribute-unit">({{ $attribute->unit }})</span>
                                    @endif
                                </div>
                                <div class="attribute-value">{{ $valueText }}</div>
                            </div>
                            <button type="button" class="attribute-remove-btn" data-attribute-id="{{ $attribute->id }}">
                                <i class="fas fa-trash"></i>
                            </button>
                        </div>
                    @empty
                        <div class="no-attributes">
                            <p>{{ __('No attributes added yet. Click "Add Attribute" to add one.') }}</p>
                        </div>
                    @endforelse
                </div>

                <!-- Add Attribute Form (Hidden by default) -->
                <div class="add-attribute-form" id="add-attribute-form" style="display: none; margin-top: 1.5rem;">
                    <div class="settings-grid">
                        <div class="settings-group">
                            <label class="settings-label required">{{ __('Attribute Name') }}</label>
                            <input type="text" class="settings-input" id="attribute-name" 
                                placeholder="{{ __('e.g., Color, Size, Weight') }}">
                        </div>
                        <div class="settings-group">
                            <label class="settings-label">{{ __('Value') }}</label>
                            <input type="text" class="settings-input" id="attribute-value" 
                                placeholder="{{ __('e.g., Red, Large, 500') }}">
                        </div>
                        <div class="settings-group">
                            <label class="settings-label">{{ __('Unit') }}</label>
                            <input type="text" class="settings-input" id="attribute-unit" 
                                placeholder="{{ __('e.g., kg, cm, ml') }}">
                        </div>
                    </div>
                    <div class="settings-actions" style="margin-top: 1rem;">
                        <button type="button" class="settings-save-btn" id="save-attribute-btn">
                            <i class="fas fa-save"></i>
                            {{ __('Save Attribute') }}
                        </button>
                        <button type="button" class="btn-secondary" id="cancel-attribute-btn" style="margin-left: 0.5rem;">
                            {{ __('Cancel') }}
                        </button>
                    </div>
                </div>
...
@push('scripts')
    ...
    <script>
            ...
            function toggleProviderFields(linkingType) {
                if (linkingType === 'automatic') {
                    ...
                    if (providerProductSelect) {
                        ...
                        providerProductSelect.innerHTML =
                            '<option value="">{{ __('Choose a product...') }}</option>';
                    }
                }
            }
                    ...
                    fetch(`{{ route('providers.products', ['provider' => ':providerId']) }}`.replace(
                        ...
                        .then(data => {
                            ...
                            if (data.success && data.products && Array.isArray(data.products) && data
                                ...
                                const currentProviderProductNumber =
                                    '{{ old('provider_product_number', $product->provider_product_number) }}';
                                ...
                                data.products.forEach(product => {
                                    ...
                                    if (currentProviderProductNumber && product.value ==
                                        currentProviderProductNumber) {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const attributeUnitInput = document.getElementById('attribute-unit');
            ...
            if (saveAttributeBtn) {
                saveAttributeBtn.addEventListener('click', function() {
                    ...
                    const unit = attributeUnitInput.value.trim();

                    if (!name) {
                        alert('{{ __('Attribute name is required') }}');
                        return;
                    }

                    // Disable button and show loading
                    const originalText = saveAttributeBtn.innerHTML;
                    saveAttributeBtn.disabled = true;
                    saveAttributeBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> {{ __('Saving...') }}';

                    // Send AJAX request
                    fetch(`/dashboard/products/${productId}/attributes`, {
                        method: 'POST',
                        headers: {
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            const successMessage = data.message || '{{ __('Attribute added successfully') }}';
                            if (typeof showSuccess === 'function') {
                                showSuccess('{{ __('Success') }}', successMessage);
                            } else {
                                // Fallback notification
                            ...
                            // Reload page after a short delay to show new attribute
                            setTimeout(() => {
                                location.reload();
                            }, 1500);
                        } else {
                            alert(data.message || '{{ __('An error occurred while adding the attribute') }}');
                            saveAttributeBtn.innerHTML = originalText;
                            saveAttributeBtn.disabled = false;
                        }
                    })
                    .catch(error => {
                        console.error('Add attribute error:', error);
                        alert('{{ __('An error occurred while adding the attribute') }}');
                        saveAttributeBtn.innerHTML = originalText;
                        saveAttributeBtn.disabled = false;
                    });
                });
            ...
            // Remove attribute
            const removeAttributeBtns = document.querySelectorAll('.attribute-remove-btn');
            removeAttributeBtns.forEach(btn => {
                btn.addEventListener('click', function() {
                    ...
                    const attributeName = attributeItem.querySelector('.attribute-name strong').textContent;

                    if (!confirm(`{{ __('Are you sure you want to remove the attribute') }} "${attributeName}"?`)) {
                        return;
                    }
                    ...
                    // Send AJAX request
                    fetch(`/dashboard/products/${productId}/attributes/${attributeId}`, {
                        method: 'DELETE',
                        headers: {
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                            'Accept': 'application/json'
                    ...
                    .then(data => {
                        if (data.success) {
                            // Remove attribute item from DOM
                            attributeItem.remove();
                            
                            // Show message if no attributes left
                            if (attributesList.querySelectorAll('.attribute-item').length === 0) {
                            ...
                            alert(data.message || '{{ __('An error occurred while removing the attribute') }}');
                            this.innerHTML = originalText;
                            this.disabled = false;
                        }
                    })
                    .catch(error => {
                        ...
                        alert('{{ __('An error occurred while removing the attribute') }}');
                        this.innerHTML = originalText;
                        this.disabled = false;
                    });
                });
